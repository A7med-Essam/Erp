...
@Component({
  ...
  styleUrls: ["./unrestrict.component.scss"],
})
export class UnrestrictComponent {
  ...
  constructor(
    public _dialogRef: MatDialogRef<UnrestrictComponent>,
  ...
  filterDeliveries() {
    return this.data.plan.filter((plan) => {
      return plan.meals.every(
        (meal) => meal.deliveryStatus == DeliveryStatusEnum.Resticited
      );
    });
  }
  ...
  onSubmit(form: FormGroup) {
    if (form.valid) {
      if (this.currentSelectedRows.length) {
        ...
        this._ActionsService.Unrestrict(form.value).subscribe({
          next: () => {
