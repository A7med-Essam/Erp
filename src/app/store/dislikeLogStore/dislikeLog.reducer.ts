...
import { IDislikeLog } from "src/app/interfaces/log.interface";

export interface IState extends IHttpResponse, IRequestStatus {
  data: IDislikeLog[] | null;
}
...
export const Reducer = createReducer(
  initalValue,
  on(fromActions.GET_DISLIKE_LOG_START, (state) => ({
    ...state,
    loading: true,
    error: null,
    succeeded: false,
  })),
  on(fromActions.GET_DISLIKE_LOG_SUCCESS, (state, action) => ({
    ...state,
    error: null,
    loading: false,
  ...
  on(fromActions.GET_DISLIKE_LOG_FAILED, (state, action) => ({
    ...state,
    error: action.error,
    loading: false,
    data: null,
